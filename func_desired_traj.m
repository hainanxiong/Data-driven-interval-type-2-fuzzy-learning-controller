...
elseif strcmp(traj_type, 'circle')
    x = 0.8 * cos(0.5 * pi * t * 1 / traj_frequency);
    y = 0.8 * sin(0.5 * pi * t * 1 / traj_frequency);
...
elseif strcmp(traj_type, 'yd')
    
    x = t;
%     y = sin(0.5 * pi * t) + (1/3) * (3 - sin(1.5 * pi * t));
    y = sin(t);

    x = x(1:2*val_length+1);
    y = y(1:2*val_length+1);
    x = normalize(x, 'range', [-0.3, 0.3]);
    y = normalize(y, 'range', [-0.1, 0.1]);
...
%% Desired reference
%the angular positions of the two arms%
...
if plot_figures_inside == 0
    figure();
    hold on
    plot(q_control(2:val_length, 1),'Color',[0.3098    0.2902    0.2706],'LineWidth',2)
    plot(q_control(2:val_length, 2),'Color',[0.6549    0.8275    0.5922],'LineWidth',2)
    ylabel('$q$')
    legend('$q_1$', '$q_2$')
    
    figure();
    hold on
    plot(q2dt_control(1:val_length-1, 1),'Color',[0.9765    0.7098    0.4471],'LineWidth',2)
    plot(q2dt_control(1:val_length-1, 2),'Color',[0.7608    0.8510    1.0000],'LineWidth',2)
    ylabel('$\dot{q}$')
    legend('$\dot{q_1}$', '$\dot{q_2}$')
    
    figure();
    hold on
    plot(qdt_control(1:val_length-1, 1),'Color',[0.8784    0.6824    0.8157],'LineWidth',2)
    plot(qdt_control(1:val_length-1, 2),'Color',[0.8392    0.8314    0.4275],'LineWidth',2)
    ylabel('$\ddot{q}$')
    legend('$\ddot{q_1}$', '$\ddot{q_2}$')
    
    figure();
    hold on
    plot(tau_control(2:val_length, 1),'Color',[0.4078    0.4941    1.0000],'LineWidth',2)
    plot(tau_control(2:val_length, 2),'Color',[0.8706    0.5608    0.3725],'LineWidth',2)
    ylabel('$\tau$')
    legend('$\tau{1}$', '$\tau{2}$')
end
